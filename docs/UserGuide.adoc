...
* The substring must be within the first or last name only of a person involved, otherwise an empty list will be
returned. +
...
or deletes the specified transaction from the past transaction list only.
...
Depending on the format used, selects the transaction identified by its index number in either the current transaction
list, or in the past transaction list. +
